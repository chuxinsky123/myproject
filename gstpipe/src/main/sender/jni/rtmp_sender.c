...
int set_vcodec(int width, int height, uint8_t *spspps, int spspps_size)
{
    ...
    if(320 == height){
        dest->bit_rate = 409600;   //320p  比特率     320
    } else if(360 == height){
        dest->bit_rate = 512000;   //480p  比特率     360
    }else{
        dest->bit_rate = 921600;   //720p  比特率     480
    }
...
static int open_output2(void *data) {
     const char *filename = (const char *) data;  //推流地址
     int ret = 0;
#if LOCAL_FILE
...
static void *thread_send_func(void *data) {
    ...
    APPLOGI("%s %d now ", __FUNCTION__, __LINE__);
        ...
        APPLOGE(" wait rtmp connect ");     //等待rtmp 流媒体链接
        av_usleep(50000);
    }
     ...
     while (1) {
         ...
         while(1) {
             if (audioq.nb_packets > 100 || videoq.nb_packets > 60) {
                ...
                int64_t adiff = mSender->send_apts - sys_time/1000;    //毫秒
                ...
                if (videoq.nb_packets > 30 && vdiff < -2000 && mSender->need_drop_vpkt == 0) {
                    ...
                    if (vpkt->pts - sys_time/1000 < -2000) {   //
                        APPLOGI("need_drop_vpkt  %lld %lld %d-%d", sys_time/1000, mSender->send_vpts, videoq.nb_packets, audioq.nb_packets);
            ...
            AVPacket *vpkt = packet_queue_peek(&videoq);
            //视频的时间戳大于音频的时间戳  根据时间戳来取出数据
            if ( apkt != NULL && (vpkt ==NULL || (apkt->pts <= vpkt->pts)) ) {    //音频的avpacket 不为NULL    视频的不为空  音频的时间戳pts大于视频的pts
        ...
        int64_t now_time = get_system_time();
         ...
         if ( mSender->need_drop_vpkt > 0 && (pkt.stream_index == mSender->videoindex) ) {
            ...
            if ( pkt.flags == 1 && mSender->need_drop_vpkt > 1 ) {
                ...
                APPLOGI("dv %lld %d", pkt.pts, pkt.size);   // pts时间戳
...
int start_send_data() {
    ...
    pthread_create (&mSender->send_thread, NULL, &thread_send_func, NULL);                     //发送线程
    pthread_create (&mSender->send_monitor_thread, NULL, &thread_send_monitor_func, NULL);     //判断发送延迟/阻塞
...
//        pthread_join(mSender->rtmp_init_thread ,&status);
    if (mSender->send_thread_created) {
        mSender->send_thread_created = 0;
        pthread_join(mSender->send_thread,&status);   //已阻塞的方式等待线程（发送线程）结束
    }
