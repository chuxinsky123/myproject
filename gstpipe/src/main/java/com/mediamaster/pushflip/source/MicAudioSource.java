...
public class MicAudioSource extends AudioSource {
    ...
    private static final int BIT_RATE = 64000;          // 64kbps  128kbps  192kbps  256kpbs
    //private static final int BIT_RATE = 128000;          // 64kbps  128kbps  192kbps  256kpbs
...
//    ArrayList<MyEnqueueSampleInfo> mMyEnqueueFrameInfoQueue = new ArrayList<MyEnqueueSampleInfo>();
    ...
    private void prepareEncoder() throws IOException {
       // MIMETYPE_AUDIO_AAC
        ...
        final MediaFormat audioFormat = MediaFormat.createAudioFormat(MediaFormat.MIMETYPE_AUDIO_AAC, SAMPLE_RATE, 1);   //采样频率 44100
        audioFormat.setInteger(MediaFormat.KEY_AAC_PROFILE, MediaCodecInfo.CodecProfileLevel.AACObjectLC);
        audioFormat.setInteger(MediaFormat.KEY_CHANNEL_MASK, AudioFormat.CHANNEL_IN_MONO);//采样精度为16
        audioFormat.setInteger(MediaFormat.KEY_BIT_RATE, BIT_RATE);
        audioFormat.setInteger(MediaFormat.KEY_CHANNEL_COUNT, 1);   //声音通道为1
        //audioFormat.setInteger(MediaFormat.KEY_CHANNEL_COUNT, 2);   //声音通道为1

        Log.i(TAG, "......Audio format: " + audioFormat);
...
//                    + ", offset=" + aBufferInfo.offset);
    ...
    protected  void write_file_aac() {
        ...
        //创建文件夹及文件
        file_02 = new File(s_s + "/AAC_Data/", "Acc_Demo.aac");
...
//        }
    ...
    protected  void write_file_aac02() {
        String s_s =  Environment.getExternalStorageDirectory().getPath();
        //创建文件夹及文件
        file_03 = new File(s_s + "/AAC_Data/", "Acc_Demo_02.aac");
        if (!file_03.exists()) {
            try {
                //按照指定的路径创建文件夹
        ...
        while (!mQuit.get() && ix < length) {
            try {
                ...
                if (inputBufferIndex >= 0) {
                    ...
                    //final ByteBuffer inputBuffer = mAudioEncoder.getInputBuffer(inputBufferIndex);
                    inputBuffer.clear();
